...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contactez l'équipe de Kolori RH pour toute question ou demande de partenariat." },
    ],
  }),
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 bg-white border border-border p-6 md:p-8 rounded-3xl shadow-sm">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="nom" className="font-semibold text-xs text-muted-foreground">Nom</Label>
                <Input id="nom" required className="rounded-xl border border-border mt-1" />
              </div>
            </div>
            <div>
              <Label htmlFor="email" className="font-semibold text-xs text-muted-foreground">Email</Label>
              <Input id="email" type="email" required className="rounded-xl border border-border mt-1" />
            </div>
            <div>
              <Label htmlFor="sujet" className="font-semibold text-xs text-muted-foreground">Sujet</Label>
              <Input id="sujet" required className="rounded-xl border border-border mt-1" />
            </div>
            <div>
              <Label htmlFor="message" className="font-semibold text-xs text-muted-foreground">Message</Label>
              <Textarea id="message" required rows={5} className="rounded-xl border border-border mt-1" />
            </div>
            <div className="pt-2">
              <Button type="submit" className="rounded-full px-8 py-3 bg-primary text-white font-semibold shadow-md hover:brightness-110">
                Envoyer le message
              </Button>
            </div>
          </form>
