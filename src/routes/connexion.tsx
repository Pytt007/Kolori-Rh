...
export const Route = createFileRoute("/connexion")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Connectez-vous à votre espace candidat ou recruteur sur Kolori RH.",
      },
    ],
  }),
...
function ConnexionPage() {
  ...
  return (
    ...
    >
      ...
      {/* Bouton reset données démo */}
      <button
        type="button"
        onClick={() => {
          const mockKeys = Object.keys(localStorage).filter((k) => k.startsWith("mock_"));
          mockKeys.forEach((k) => localStorage.removeItem(k));
          window.location.reload();
        }}
        className="w-full text-center text-[10px] font-mono tracking-widest text-muted-foreground/60 hover:text-muted-foreground transition-colors mt-1 py-1"
      >
