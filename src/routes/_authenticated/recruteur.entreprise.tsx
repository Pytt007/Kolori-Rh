...
function RecruteurEntreprise() {
  ...
  const [form, setForm] = useState<Form>({ nom: "", secteur: "", localisation: "", site_web: "", description: "", logo_url: null });
  ...
  async function save(e: React.FormEvent) {
    ...
    if (!id) {
      const { data } = await supabase.from("companies").select("id, statut").eq("owner_id", user.id).maybeSingle();
      if (data) { setId(data.id); setStatut(data.statut); }
    }
  }
    ...
    <>
      ...
      {id && (
        <div className="mb-8 flex items-center gap-3">
          <span className="text-xs font-mono uppercase tracking-widest text-muted-foreground">Statut :</span>
          <span className={`text-xs font-mono uppercase tracking-widest px-2 py-1 rounded-sm ${COMPANY_STATUS_LABELS[statut]?.tone}`}>
      ...
      {!id && (
        <p className="text-muted-foreground mb-8 max-w-2xl">
          Renseignez les informations de votre entreprise. Une fois enregistrées, elles seront soumises à validation par notre équipe.
        </p>
      )}
      ...
      <form onSubmit={save} className="max-w-2xl space-y-6">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <Button type="button" variant="outline" onClick={() => fileRef.current?.click()}>Téléverser un logo</Button>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input id="site_web" type="url" placeholder="https://" value={form.site_web} onChange={(e) => setForm({ ...form, site_web: e.target.value })} />
          </div>
        </div>
        ...
        <Button type="submit" disabled={saving}>{saving ? "Enregistrement…" : id ? "Enregistrer" : "Créer la fiche"}</Button>
      </form>
    </>
  );
}
