...
import { MOCK_JOB_OFFERS, MOCK_COMPANIES, getMockJobOffers, getMockCompanies } from "@/lib/mockData";
...
type OfferDetail = {
  ...
  company: { nom: string; secteur: string | null; logo_url: string | null; localisation: string | null } | null;
};
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="text-center">
        ...
        <Link to="/offres" className="text-primary underline">Voir toutes les offres</Link>
      </div>
    </div>
  ),
    ...
    (async () => {
      ...
      try {
        const { data, error } = await supabase
          .from("job_offers")
          .select("id, titre, description, contrat, localisation, teletravail, salaire_min, salaire_max, publiee_le, secteur, competences_requises, company:companies(nom, secteur, logo_url, localisation)")
          .eq("id", offerId)
          .eq("statut", "publiee")
          .maybeSingle();
        ...
        if (!finalOffer) {
          ...
          try {
            ...
            if (related && related.length > 0) {
              ...
              const mockRelated = getMockJobOffers().filter(o => o.id !== offerId).slice(0, 3);
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-4 text-xs font-bold text-white/80 mt-2 uppercase tracking-wider">
                      <span>Publiée le {offer.publiee_le ? new Date(offer.publiee_le).toLocaleDateString("fr-FR") : "Récemment"}</span>
            ...
            <section className="max-w-7xl mx-auto px-6 py-12 grid lg:grid-cols-12 gap-10 items-start">
              ...
              <div className="lg:col-span-4 bg-white border border-border rounded-2xl p-6 shadow-sm">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed mb-6">
                  {offer.company ? (
                    <span>
                      Une entreprise de premier plan du secteur <strong>{offer.company.secteur || "activité"}</strong> recrute actuellement un profil qualifié de <strong>{offer.titre}</strong> pour renforcer ses équipes à {offer.localisation || "Abidjan"}.
                    </span>
                  ) : (
                    <span>
                      Nous recrutons actuellement pour le poste de {offer.titre}. Consultez la description complète et déposez votre candidature ci-dessous.
                    </span>
                  )}
                </p>
                ...
                <ul className="space-y-3.5 text-xs mb-8">
                  {offer.localisation && (
                    <li className="flex items-center gap-2">
                      ...
                      <span><strong>Localisation :</strong> {offer.localisation}</span>
                    </li>
                  )}
                  {offer.teletravail && (
                    <li className="flex items-center gap-2">
                      <Briefcase className="w-4 h-4 text-primary shrink-0" />
                      <span><strong>Télétravail :</strong> {offer.teletravail}</span>
                    </li>
                  )}
                  {(offer.salaire_min || offer.salaire_max) && (
                    <li className="flex items-center gap-2">
                      <Euro className="w-4 h-4 text-primary shrink-0" />
                      <span><strong>Salaire :</strong> {offer.salaire_min ?? "?"}k – {offer.salaire_max ?? "?"}k / an</span>
                    </li>
                  )}
                </ul>
                  ...
                  {isAuthenticated && hasRole("candidat") ? (
                    ...
                    <>
                      <Link
                        ...
                        className="w-full inline-flex items-center justify-center bg-primary text-primary-foreground py-3 text-sm font-semibold rounded-full shadow-md hover:brightness-110 transition-all text-center"
                      >
                        Postuler maintenant
                      </Link>
                      <Link 
                        to="/connexion" 
                        className="w-full inline-flex items-center justify-center bg-white border border-border text-foreground py-3 text-sm font-semibold rounded-full hover:bg-secondary transition-colors text-center"
                      >
                        Se connecter
                      </Link>
                    </>
            ...
            <section className="max-w-7xl mx-auto px-6 py-8">
              <div className="bg-accent text-accent-foreground rounded-3xl p-10 md:p-12 shadow-xl flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                ...
                <div className="space-y-2 z-10 text-center md:text-left">
                  <span className="text-xs uppercase tracking-widest text-accent-foreground/80 font-bold">RESTEZ EN CONTACT</span>
                  <h2 className="text-2xl md:text-3xl font-display font-bold">Des questions sur ce poste ?</h2>
                  <p className="text-sm opacity-90">Notre équipe de consultants en recrutement est là pour vous guider.</p>
                </div>
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-4">
                          <span className="text-[10px] font-bold text-primary uppercase tracking-wider">
                            {ro.publiee_le ? new Date(ro.publiee_le).toLocaleDateString("fr-FR") : "Récemment"}
                          </span>
